...
from helper import PROMPT_TEMPLATE_HYDE, extract_officer_data, generate_hypothetical_embeddings, sort_retrived_documents
...
def get_response_from_query(db, query, temperature, k):
    ...
    llm = ChatAnthropic(model_name="claude-3-sonnet-20240229", temperature=temperature)
...
def process_file(file_name, input_path, output_path, file_type, embeddings):
    ...
    logger.info(f"CSV output for {file_name} saved successfully.")

def process_files(input_path, output_path, file_type, embeddings):
    for file_name in os.listdir(input_path):
        if file_name.endswith(".json"):
            process_file(file_name, input_path, output_path, file_type, embeddings)

def process_query(input_path_transcripts, input_path_reports, output_path):
    embeddings = generate_hypothetical_embeddings()
    process_files(input_path_transcripts, output_path, "transcript", embeddings)
    process_files(input_path_reports, output_path, "report", embeddings)

if __name__ == "__main__":
    input_path_transcripts = r"../../ocr/data/output/transcripts"
    input_path_reports = r"../../ocr/data/output/reports"
    output_path = r"../data/ner-20-pages"
